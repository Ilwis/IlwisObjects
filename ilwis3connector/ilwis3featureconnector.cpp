...
void FeatureConnector::writeLine(const geos::geom::LineString* line,std::ofstream& output_file,long raw ) {
    ...
    int noOfCoordsBytes = (quint32)line->getNumPoints() * 16;
            ...
            case geos::geom::GEOS_MULTIPOINT:
                {
                    ...
                    points += (quint32)geometries->getNumGeometries();
                }
                break;
            case geos::geom::GEOS_MULTILINESTRING:
                {
                    const geos::geom::Geometry *geometries = geometry.get();
                    segments += (quint32)geometries->getNumGeometries();
                }
                break;
            case geos::geom::GEOS_MULTIPOLYGON:
                {
                    const geos::geom::Geometry *geometries = geometry.get();
                    polygons += (quint32)geometries->getNumGeometries();
                }
                break;
